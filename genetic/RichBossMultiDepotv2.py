...
"""
MultiDepotRichBoss v2 pseudo code (onini 27.06.2021)
...
f- Let "Rsac+(d)" matrix is additional access times when routes sacrifices itself. obtained from Rsac(d)
        ...
        for depot d in depotList: # NEW NEW NEW 
            for every index x in  R: 
                ...
                for every index y in R:
                    newRouting(d) = addToRoot(d, r(y),r(sac))  /*join r(sac) to r(y) */
