...
Rich Boss aproach
intuition: VRP Company have a "Rich" boss. And boss buy a vehicle for every clients.Then things are starting to go bad by one by one and our boss isnt "Rich" anymore. 
...
d- Let we have "V" vehicle and we want obtain same number of routes
for optimum routing
...
class RichBoss:
    ...
    def __init__(self, costn:List, V:int, demands:list, capacity) -> None:
        self.costn = costn
            ...
            for sac in range(R):    
                ...
                for y in range(R): #for every index y  in R:
                    ...
                    if sac == y: # same route dont calculate
                        newAccessTime = self.calculateNewAccessTime(self.r[sac],len(self.r[sac])-1-1) #extra minus for depot
                        ...
                        newAccessTime = self.calculateNewAccessTime(newRouting.get('newRoute'),len(newRouting.get('newRoute'))-1-1) 
                        newTotalDemands = self.calculateTotalDemands(newRouting.get('newRoute'))
                        #Rsac[y][sac] = newAccessTime, newRouting /*Rsac[y][sac] = objectiveValue*/
                        self.Rsac[sac][y] = {'newAccessTime':newAccessTime,'newRoute':newRouting.get('newRoute'), 'newTotalDemands':newTotalDemands}
    ...
    def selectMinFromRsacPlus(self, rsacPlus:List)->Dict:
        ...
        for indx,accessPlusList in enumerate(rsacPlus):
            for indy in range(len(accessPlusList)):
                if indx != indy and rsacPlus[indx][indy].get('plusAccessTime') < minVal and rsacPlus[indx][indy].get('totalDemands')  <= self.capacity:
    ...
    def printPlotSolution(self,coordinate_points, routes):
        totalDist = 0
        tours =[]
        for r in routes:
            totalDist += self.calculateNewAccessTime(r,len(r)-1)
            print(r,"-> total distance -access time:",self.calculateNewAccessTime(r,len(r)-1),"-",self.calculateNewAccessTime(r,len(r)-2),
...
if __name__ == '__main__':
    ...
    rich = RichBoss(costn,route_num, demands,capacity)

    #a = [0,3,4,0]
    #b = [0,10,8,6,0]    
    #c = rich.mergeRoutesWithNearestEdge(a,b)
    #print('c:',c)
    #print('a:',a)
    #print('b:',b)
    """
    for ind,r in enumerate(rich.r):
        print("r(",ind+1,")->",r)
    """
    #rich.addToRoute(rich.r[2-1],rich.r[1-1])        
    routes = rich.run()
    """
    for ind,r in enumerate(rich.Rsac):
        print("rsac(",ind+1,")->",r)
    ...
    for r in routes:
        totalDist += rich.calculateNewAccessTime(r,len(r)-1)
        print(r,"-> total distance:",rich.calculateNewAccessTime(r,len(r)-1),
